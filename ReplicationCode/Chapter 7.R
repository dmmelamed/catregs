...
list.coef(m1)$out[1,3]^2 # Wald-statistic (z-value squared)
pchisq(list.coef(m1)$out[1,3]^2,1,lower.tail=FALSE) # p-value on the Wald-test
...
summary(marginaleffects(m1,variables="female"))
summary(marginaleffects(m1,variables="female",newdata=datagrid(religious=1))) # conditional AME
summary(marginaleffects(m1,variables="female",newdata=datagrid(religious=6))) # conditional AME

mef <- data.frame(summary(marginaleffects(m1,variables="female")))
mef$group <- factor(mef$group,levels=c("Very unsafe","Unsafe","Safe","Very safe"))
# Figure 7.10
...
mef <- data.frame(summary(marginaleffects(m1,variables="female",newdata=datagrid(religious=1))))
mef2 <- data.frame(summary(marginaleffects(m1,variables="female",newdata=datagrid(religious=6))))
mef<-rbind(mef,mef2)
...
pdat
pdat$dv <- pdat2$walk.alone.dark
p2 <-ggplot(pdat,aes(x=age,y=prob,ymin=ll,ymax=ul,group=as.factor(dv),linetype=as.factor(dv),color=as.factor(dv))) +
  theme_bw() + geom_line() + geom_point() + geom_ribbon(alpha=.1) +
  scale_color_manual(values=natparks.pals("DeathValley")) + labs(x="Age",y="Probability",linetype="",color="")
require(ggpubr)
ggarrange(p2,p1,common.legend = TRUE,legend="bottom",labels=c("A","B"))
...
ggplot(pdat,aes(x=xaxs,y=prob,ymin=ll,ymax=ul,fill=female)) +
  ...
  scale_x_continuous(breaks=c(.025,.175,.325,.475),labels=c("Very Unsafe","Unsafe","Safe","Very Safe")) +
  scale_fill_manual(values=c("grey65","grey35"))
p1<-ggplot(pdat,aes(x=xaxs,y=prob,ymin=ll,ymax=ul,fill=female)) +
  theme_bw() + geom_col() + labs(x="",y="Predicted Probabilities",fill="") +
  theme(legend.position="bottom") + geom_errorbar(width=.02) +
  scale_x_continuous(breaks=c(.025,.175,.325,.475),labels=c("Very Unsafe","Unsafe","Safe","Very Safe")) +
  scale_fill_manual(values=c("grey65","grey35"))
...
pdat <- mutate(pdat,xaxs=c(0,.15,.3,.45,.05,.2,.35,.5),
               female=rep(c("Male","Female"),each=4))
p2<-ggplot(pdat,aes(x=xaxs,y=prob,ymin=ll,ymax=ul,fill=female)) +
  theme_bw() + geom_col() + labs(x="",y="Predicted Probabilities",fill="") +
  theme(legend.position="bottom") + geom_errorbar(width=.02) +
  scale_x_continuous(breaks=c(.025,.175,.325,.475),labels=c("Very Unsafe","Unsafe","Safe","Very Safe")) +
  scale_fill_manual(values=c("grey65","grey35"))
ggarrange(p1,p2,common.legend = TRUE,legend="bottom",labels=c("A","B"))
